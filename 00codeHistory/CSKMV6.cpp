 ...
 * Version 2
        ...
        2- Adding some print to stdout for more information.
	...
	version 1
		1- start besed on Zadehgol D2Z6 code.
*/

// ---------------------------------  the adjustable parameters --------------------------------

#define BS                 47              //  grid size (increase BS for finer grids) 
											// 47 for 129*129
											// 74 for 201*201
											// 146 for 401*401
											
#define Re                 100.0          //  Reynolds nunber 
...
#define DesiredError                0.00001         // 1*10^-5 for RMS_Error


#define MakeErrorCSVFile        // if defined will make Error CSV File if comment will not make it 
//#define MakeMultipleFile        // if defined will make Multiple output file if comment will not make them.
...
{ 
    ...
    int GRID_SIZE = x_dim - y_dim/2;
        ...
        // put "file" name in the string  it makes file name in the string each time.
        snprintf(fileNameString4,sizeof(char)*64,"0_Errors_G(%d)_U(%1.4f)_R(%2.0f).CSV",GRID_SIZE,U,Re);
        FILE *ErrorCSV;
    ...
    while (RMS_Error > DesiredError || iter <= (d_iter_01*2 + 1))
    {
		double y0, T;
