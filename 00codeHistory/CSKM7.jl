...
const BS =                1              #  grid size (increase BS for finer grids)
const MACH   =            0.39           #  Mach number (try the 0.025 - 0.10 range)
...
const Re    =             0.0          #  Reynolds nunber (try the 100 - 10,000 range)




const StabilityRun	=  "True" # if defined will break at MAX_UnSteady_Itteration
const MAX_UnSteady_Itteration =    100001         # Max ittertation
const DesiredError          =      0.00001         # 1*10^-5


#const MakeErrorCSVFile        # if defined will make if comment will not make
#const MakeMultipleFile        # if defined will make if comment will not make


const d_iter_01   =       1  # error calc intervals
const d_iter_02   =       1000000  # File Outputs intervals
const d_iter_03   =       5000	 # showing error intervals
...
const y_dim     =         BS*28	# originally 77
const x_dim     =         BS*38	# originally 105
...
function main()
    ...
    iter = 0;
    int i,j;
    F_in = Array{Float64}(x_dim,y_dim,6);
    F_out = Array{Float64}(x_dim,y_dim,6);
    bdr_state = Array{Int32}(x_dim,y_dim);
