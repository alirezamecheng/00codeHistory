...
#/==============================================================================
#=  
    * version 27-2 based on 27 
        1- changed the RMS error calculation to L2 norm error calculation (starts on line 1206).
...
const Grid = 1 + 2^5          # actual number of nodes including the boundary nodes. (Grid == Nx1). 
const Nx = Grid - 1       # number of cells in the x-direction -1
const Ny = Grid - 1       # number of cells in the y-direction -1
const Re = 100.0
const errorlimitation = 1.0e-7;
const ErrCalculationSteps = 1000;
const MAX_UnSteady_Itteration = 500000;     # is not active in this version 
const MaxItteration = 1000;                 # is not active in this version
...
function ErrorManager!(ux::Array{Float64},
    ...
    # L2 Norm Error calculations
    
    nom::Float64=0;
    den::Float64=0;
    @inbounds for c = 2:Nx
        for r = 2:Ny # حدود محاسبه نود هایی هست که روی آنها عملیات مرزی انجام نشده است
            nom = nom + sqrt((ux[r,c] - u0[r,c])^2 + (uy[r,c] - v0[r,c])^2);
            den = den + sqrt((ux[r,c])^2 + (uy[r,c])^2);
        end 
    end
   ...
   @inbounds for c = 2:Nx
    ...
    # RMS_Error = sqrt(s/((Nx-1)*(Ny-1))); # old Return 
    # new return 
