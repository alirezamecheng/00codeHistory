...
#/==============================================================================
#=
    * version 31 based on 30
        1- adding relaxation times vector to NOCMS collision function call.

    * version 30 bsded on 29
        1- bug fix in plt output function.
...
function Coll_CMs_Non_Orthogonal!(rho::Array{Float64}, ux::Array{Float64}, uy::Array{Float64}, f::Array{Float64}, f_post::Array{Float64}, s::Array{Float64})
...
g8 = (omega8/4)*(RHO/9 + 3*RHO*(Ux^2)*(Uy^2) - (kxxyy - 2*Ux*kxyy - 2*Uy*kxxy + (Ux^2)*kyy + (Uy^2)*kxx + 4*Ux*Uy*kxy)) -2*g3 -0.5*(Uy^2)*(3*g3 + g4) - 0.5*(Ux^2)*(3*g3 - g4) - 4*Ux*Uy*g5 - 2*Uy*g6 - 2*Ux*g7;
...
# ================= Allocations for Error function =============================
   ...
   # ================= NOCMS Special Variables and allocations ===============

    @static if MethodName == "CMs_NON_Orthogonal" # static elseif
    s       = Array{Float64}(undef,Q);                 # Relaxation rates array
    tauNOCMS::Float64  = 3*Lenght*uw_top/Re + 0.5;   # relaxation time for BGK
    s[9] = 1.0;
    s[1] = 1.0;
    s[2] = 1.0;
    s[3] = 1.0;
    ...
    s[7] = 1.0;
    s[8] = 1.0;
    end # static if [MethodName == "CMs_NON_Orthogonal"]
...
#         f_post = Coll_CMs_Non_Orthogonal!(rho, ux, uy, f, f_post, tau); # CMs Non-Orthogonal collision
        f_post = Coll_CMs_Non_Orthogonal!(rho, ux, uy, f, f_post, s); # CMs Non-Orthogonal collision
        f      = Streaming!(f,f_post);                                  # Streaming for BGK
        f      = Bounce_back!(f,f_post,rho);                            # No-Slip boundary condition
        rho, ux, uy = Den_vel!(rho,ux,uy,f);                            # Fluid variables calculation for all models
