...
# const MethodName = "CMs_NON_Orthogonal";  # CMs. Non-Orthogonal Central Moments space
 const MethodName = "CascadedLBM"            # cascaded Central moments space ATTENTION VISCOSITYS ARE INCOMPLETE
...
@static if MethodName == "CascadedLBM"
    ...
    function Ktransform!(RHO::Float64,
        ...
        kxxy   = (-1*((se + sw - ne -nw - 2*Ux^2*Uy*RHO + Uy*(RHO - n - s -r))/4 + Ux/(2*(ne -nw - se + sw)))) - Uy/(2*(-3*kxxpyy - kxxmyy)) - 2*Ux*kxy;
        kxyy   = (-1*((sw + nw - se - ne - 2*Uy^2*Ux*RHO + Ux*(RHO - w -e -r))/4 + Uy/(2*(ne + sw - se -nw)))) - Ux/(2*(-3*kxxpyy + kxxmyy)) - Uy*kxy;
        kxxyy  = (1/(4*(RHO/9 - ne -nw -se - sw + 2*(Ux*(ne - nw + se - sw) + Uy*(ne + nw - se - sw)))) + 4*Ux*Uy*(nw - ne + se - sw) - Ux^2 * (n + ne + nw + s + se + sw) + Uy^2 * (3*Ux^2 * RHO - e -ne - nw - se - sw - w)) - 2*kxxpyy - 2*Ux*kxyy + 4*Ux*Uy*kxy - (3/(2*kxxpyy) - kxxmyy/2)*(Ux^2 + Uy^2);
...
@static if MethodName == "CascadedLBM"
    ...
    function CascadedLBMCollition!(rho::Array{Float64},ux::Array{Float64},uy::Array{Float64},f::Array{Float64},f_post::Array{Float64},omega_bulk::Float64,omega_shear::Float64)
        for c = 1:Nx1
            for r = 1:Ny1
                f_post[r,c,9],
                f_post[r,c,1],
...
# ================= Allocations for Error function =============================
    ...
    @static if MethodName == "MRT_Gue"  # static if MRT_Gue Method
    ...
    end # end [ static if MRT_Gue]
    
    
    # ================= CascadedLBM Special Variables and allocations ===============
    ...
    end # end if [MethodName CascadedLBM]
    ...
    # for it = 1:MaxItteration                                                            # method 3
        ...
        elseif MethodName == "CascadedLBM" # static if
        # Cascaded LBM ============================ ================================
        f_post = CascadedLBMCollition!(rho,ux,uy,f,f_post,omega_bulk,omega_shear); # Cascaded LBM collision.
        f = Streaming!(f,f_post);                                     # Streaming for BGK
        f = Bounce_back!(f,f_post,rho);                               # No-Slip boundary condition
        rho, ux, uy = Den_vel!(rho,ux,uy,f);                          # Fluid variables calculation for all models
