...
const Nx = 20        # number of cells in the x-direction -1
const Ny = 20        # number of cells in the y-direction -1
const Re = 1.0
...
#             println("err = ", err , " ux_center = ", ux[M2,N2] , " uy_center = ", uy[M2,N2] , " itt = ", LoopItterateCounter);  # Display some results
            println("err = ", err, " itt = ", LoopItterateCounter); 
            flush(STDOUT);

            if ErrorPrintingInInFilePermission == "yes"
              println(ErrorFilePointer,LoopItterateCounter,"\t",err);       # saving errors in a file pointed at with ErrorFilePointer
