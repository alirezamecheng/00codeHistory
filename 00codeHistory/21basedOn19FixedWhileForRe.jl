...
const Nx = 400        # number of cells in the x-direction -1
const Ny = 400        # number of cells in the y-direction -1
const Re = 7501.0
const errorlimitation = 1.0e-5;
const ErrCalculationSteps = 1000;
const MAX_UnSteady_Itteration = 500000;
...
@static if MethodName == "MRT_Guo" || MethodName == "MRT_EAslan" || MethodName == "MRT_Lalle" || MethodName == "MRT_Lalle2" ||  MethodName == "MRT_Khatoon"
...
@static if MethodName == "Cas_Geier" || MethodName == "Cas_Lalle" || MethodName == "Cas_Chavez"
    ...
    function Ktransform!(RHO::Float64,
        ...
        kxxpyy = omega_bulk  * ( RHO * ( Ux^2 + Uy^2 ) - e - n - s - w - 2 * ( se + sw + ne + nw - RHO/3 ))/12;
        kxxmyy = omega_shear * ( n + s - e - w + RHO*( Ux^2 - Uy^2 ))/4;
        kxy    = omega_shear * ((ne + sw - nw - se) - Ux*Uy*RHO)/4;
        kxxy   = omega6*(-((se + sw - ne -nw - 2*Ux^2*Uy*RHO + Uy*(RHO - n - s -r))/4 + Ux/2*(ne -nw - se + sw))) - Uy/2*(-3*kxxpyy - kxxmyy) - 2*Ux*kxy;
        kxyy   = omega7*(-((sw + nw - se - ne - 2*Uy^2*Ux*RHO + Ux*(RHO - w -e -r))/4 + Uy/2*(ne + sw - se -nw))) - Ux/2*(-3*kxxpyy + kxxmyy) - 2*Uy*kxy;
        kxxyy  = omega8*(1/4*(RHO/9 - ne - nw - se - sw + 2*(Ux*(ne - nw + se - sw) + Uy*(ne + nw - se - sw)) + 4*Ux*Uy*(nw - ne + se - sw) - Ux^2 * (n + ne + nw + s + se + sw) + Uy^2 * (3*Ux^2 * RHO - e -ne - nw - se - sw - w))) - 2*kxxpyy - 2*Ux*kxyy -2*Uy*kxxy + 4*Ux*Uy*kxy - (3/2*kxxpyy - kxxmyy/2)*(Ux^2 + Uy^2);

        r  = r  + 4*(-kxxpyy + kxxyy);
        e  = e  -   kxxpyy - 2*kxxyy + kxxmyy + 2*kxyy;
        n  = n  -   kxxpyy - 2*kxxyy - kxxmyy + 2*kxxy;
        w  = w  -   kxxpyy - 2*kxxyy + kxxmyy - 2*kxyy;
        s  = s  -   kxxpyy - 2*kxxyy - kxxmyy - 2*kxxy;
        ne = ne + 2*kxxpyy +   kxxyy - kxy - kxxy - kxyy;
        nw = nw + 2*kxxpyy +   kxxyy + kxy - kxxy + kxyy;
        sw = sw + 2*kxxpyy +   kxxyy - kxy + kxxy + kxyy;
        se = se + 2*kxxpyy +   kxxyy + kxy + kxxy - kxyy;
        return r, e, n, w, s, ne, nw, sw, se;
...
# ================= Allocations for Error function =============================
    ...
    .__________________________________________________________.
     =# 
        ...
        elseif MethodName == "MRT_Guo" || MethodName == "MRT_EAslan" || MethodName == "MRT_Lalle" || MethodName == "MRT_Lalle2" ||  MethodName == "MRT_Khatoon"  # static elseif
...
#             println("err = ", err , " ux_center = ", ux[M2,N2] , " uy_center = ", uy[M2,N2] , " itt = ", LoopItterateCounter);  # Display some results
            ...
            if ((Re > 7500) && (LoopItterateCounter > MAX_UnSteady_Itteration)) || (LoopItterateCounter > 1000000)
