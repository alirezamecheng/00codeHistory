...
#=
    * version 20
        1- the cascaded LMB collision operator was wrong, thanks to God damn Mr. Geier.

    * version 19
        1- equations of m_eq has been fixed. God damn Guo.
...
const Nx = 100        # number of cells in the x-direction -1
...
# const MethodName = "MRT_Lalle"            # MRT From lallemand article
# const MethodName = "MRT_Lalle2"           # MRT From lallemand seri 2
...
@static if MethodName == "MRT_Guo" || MethodName == "MRT_EAslan" || MethodName == "MRT_Lalle" || MethodName == "MRT_Lalle2"
...
#with Rho^2
    ...
    if k == 9
        x = RHO;
    elseif k == 1
        x = RHO*(-2+3*RHO*(U*U+V*V));
    elseif k == 2
        x = RHO - 3*RHO*RHO*(U*U+V*V);
    elseif k == 3
        x = RHO*U;
    elseif k == 4
        x = -RHO*U;
...
@static if MethodName == "Cas_Geier" || MethodName == "Cas_Lalle" || MethodName == "Cas_Chavez"
    ...
    function Ktransform!(RHO::Float64,
        ...
        #g0 = g1 = g2 = 0; paper of Premnath2009a

        g3 = omega3/12*(-(f1 + f2 + f3  + f4) - 2*(f5 + f6 + f7 + f8) + RHO*2/3 + RHO*(Ux^2 + Uy^2));
        g4 = omega4/4 *(f2 + f4 - f1 - f3 + RHO*(Ux^2 - Uy^2));
        g5 = omega5/4*(-f5 - f7 + f6 + f8 + RHO*Ux*Uy);
        #g6 = omega6/4*(f5 + f6 -f7 - f8 - 2*Ux*(f5 + f7 - f6 - f8) - Uy*(RHO - f9 - f2 - f4) + 2*RHO*Ux^2*Uy) - 2*Ux*g5 - 0.5*Uy*(3*g3 + g4);
        g6 = omega6/4*(f5 + f6 -f7 - f8 - 2*Ux*(f5 + f7 - f6 - f8) - Uy*(f1 + f3 + f5 + f6 + f7 + f8) + 2*RHO*Ux^2*Uy) - 2*Ux*g5 - 0.5*Uy*(3*g3 + g4);
#         g7 = omega7/4*(f5 + f8 - f6 -f7 - 2*Uy*(f5 + f7 - f6 - f8) - Ux*(RHO - f9 - f1 - f3) + 2*RHO*Ux*Uy^2) - 2*Uy*g5 - 0.5*Ux*(3*g3 - g4);
        g7 = omega7/4*(f5 + f8 - f6 -f7 - 2*Uy*(f5 + f7 - f6 - f8) - Ux*(f2 + f4 + f5 + f6 + f7 + f8) + 2*RHO*Ux*Uy^2) - 2*Uy*g5 - 0.5*Ux*(3*g3 - g4);
#         g8 = omega8/4*(-(f5 + f6 + f7 + f8 - 2*Ux*(f5 + f8 - f6 - f7) -2*Uy*(f5 + f6 - f7 - f8) + Ux^2*(RHO - f9 - f1 - f3) + Uy^2*(RHO - f9 - f2 - f4) + 4*Ux*Uy*(f5 + f7 - f6 - f8)) + RHO/9 + 3*RHO*Ux^2*Uy^2) - 2*g3 - 0.5*Ux^2*(3*g3 - g4) - 0.5*Uy^2*(3*g3 + g4) - 4*Ux*Uy*g5 - 2*Uy*g6 - 2*Ux*g7;
        g8 = omega8/4*(-(f5 + f6 + f7 + f8 - 2*Ux*(f5 + f8 - f6 - f7) -2*Uy*(f5 + f6 - f7 - f8) + Ux^2*(f2 + f4 + f5 + f6 + f7 + f8) + Uy^2*(f1 + f3 + f5 + f6 + f7 + f8) + 4*Ux*Uy*(f5 + f7 - f6 - f8)) + RHO/9 + 3*RHO*Ux^2*Uy^2) - 2*g3 - 0.5*Ux^2*(3*g3 - g4) - 0.5*Uy^2*(3*g3 + g4) - 4*Ux*Uy*g5 - 2*Uy*g6 - 2*Ux*g7;
        ...
        f3 = f3 - g3 + g4 - 2*(g7 + g8);
        f4 = f4 - g3 - g4 - 2*(g6 + g8);
        f5 = f5 + 2*g3 + g5 - g6 - g7 + g8;
        f6 = f6 + 2*g3 - g5 - g6 + g7 + g8;
        f7 = f7 + 2*g3 + g5 + g6 + g7 + g8;
        f8 = f8 + 2*g3 - g5 + g6 - g7 + g8;


        return f9, f1, f2, f3, f5, f5, f6, f7, f8;
...
# ================= Allocations for Error function =============================
    ...
    @static if MethodName == "MRT_Lalle2"  # static if MRT_Guo Method
    ...
    s[7]    = 1.9;                        # relaxation rates for MRT
    s[8]    = 1.54;
    s[9]    = 0.0;
    s[3]    = 1.98;
    s[5]    = 1.0/tauMRT;
...
#     g     = Array{Float64}(Q);
    ...
    omega3::Float64 = 1.0;
    omega4::Float64 = 1/tauCascade;
    omega5::Float64 = 1/tauCascade;
    omega6::Float64 = 1.0;
    omega7::Float64 = 1.0;
    omega8::Float64 = 1.0;
    ...
    # for it = 1:5000
        ...
        elseif MethodName == "MRT_Guo" || MethodName == "MRT_EAslan" || MethodName == "MRT_Lalle" || MethodName == "MRT_Lalle2"   # static elseif
        ...
        # Cascaded LBM ============================ ================================
        f_post = CascadedLBMCollition!(rho,ux,uy,f,f_post,omega3,omega4,omega5,omega6,omega7,omega8); # Cascaded LBM collision.
        f = Streaming!(f,f_post);                                     # Streaming for BGK
        f = Bounce_back!(f,f_post,rho);                               # No-Slip boundary condition
        rho, ux, uy = Den_vel!(rho,ux,uy,f);                          # Fluid variables calculation for all models
