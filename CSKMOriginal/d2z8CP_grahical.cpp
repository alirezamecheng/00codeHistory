/*---------------------------------------------------------------------------------------------*/
/*  Abed Zadehgol               d2z8-CP (graphical version)                 DATE : 2016/09/16  */
...
/*  time method, and it employs the Glut libarary for a graphical display of the varying       */
/*  velocity field.  Note that the apparent jumps are given by d = +/- (0.5*(sqrt(2.0)-1.0)),  */
...
// ---------------------------------   other parameters  -------------------------------
char title[] = "Sample code for the Generalized BGK-Boltzmann equation (graphics included)";  
...
#define V         (0.5*(sqrt(2.0)+1.0))   // magnitude of the microscopic velocities
...
void reset_F_in(void){
  ...
  for(i=0;i<=x_dim-1;++i){
	...
	// implementing the Zuo-He boundary condition (bounce back of the non-equilib. dist.s) 
...
void show_results(void){
    ...
    for(j=1; j<y_dim-2;++j){
	    for(i=1; i<x_dim-2;++i){
		  ...
		  xx_ctr = -0.95 + 1.5*xx/(y_dim+0.0);
		  yy_ctr = -0.80 + 1.5*yy/(y_dim+0.0);
		  ...
		  pc1 = 1.0*pc0 + 0.05*sin(10.0*pc0); 
		  pc2 = 1.5*pc0 + 0.05*sin(50.0*pc0);   
		  pc3 = 2.5*pc0 + 0.025*sin(25.0*pc0);   
...
static void Key(unsigned char key, int x, int y){ 
  if(key == 'e'){	  
	  printf("\n recording data and existing the program...\n\n");
	  rec_si();
	  rec_field();
	  exit(0);  
  }
